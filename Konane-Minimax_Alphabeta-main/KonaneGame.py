...
class KonaneGame:
    ...
    def evaluate(self, board, color, terminal_value=0):
        ...
        value += (3 * self.checkCorners(board, color))
        value -= (3 * self.checkCorners(board, self.opponent(color)))
        value += (1 * (board.count_symbol(color) - board.count_symbol(self.opponent(color))))
    ...
    def checkCorners(self, board, color):
        point = 0
        if board.game_board[0][0].piece == color: point += 1
        if board.game_board[-1][-1].piece == color: point += 1
        if board.game_board[0][board.size - 1].piece == color: point += 1
        if board.game_board[board.size - 1][0].piece == color: point += 1
        return point
